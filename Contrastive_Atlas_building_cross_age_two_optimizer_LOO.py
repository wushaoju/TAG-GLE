...
def contrastive_loss_pos_neg_sigmoid(x1, x2, temperature=0.10):
    """
    Contrastive loss using softmax over all negatives.

    Args:
    - x1: Tensor of shape (N, C, 32, 32, 32) - Positive samples
    - x2: Tensor of shape (N, C, 32, 32, 32) - Negative samples
    ...
    # Flatten spatial dimensions into 1D vectors while keeping batch size and channels
    x1_flat = x1.view(x1.size(0), x1.size(1), -1)  # Shape: (N, C, D)
    x2_flat = x2.view(x2.size(0), x2.size(1), -1)  # Shape: (N, C, D)

    # Normalize across the last dimension (L2 normalization)
    x1_flat = F.normalize(x1_flat, p=2, dim=-1)  # Shape: (N, C, D)
    x2_flat = F.normalize(x2_flat, p=2, dim=-1)  # Shape: (N, C, D)
    
    
    pairwise_mult = x1_flat.unsqueeze(1) * x2_flat.unsqueeze(0)  # Shape: (N, N, C, D
    cos_sim = torch.sum(pairwise_mult, dim=-1)
    cos_sim = cos_sim.mean(dim=-1)
    
    cos_sim_score = torch.sum(x1_flat * x2_flat, dim=-1) # Sum over the flattened spatial dimensions for each channel
    
    print(cos_sim_score.mean())
    
    loss = -torch.log(torch.sigmoid(-cos_sim)).mean()
...
def train_network(index, output_folder, trainloader, aveloader, train_restloader, net, net_rest, para, criterion, optimizer, optimizer2, at_lr, DistType, RegularityType, weight_dist, weight_reg, xDim, yDim, zDim, im_info, dev):
    #running_loss = 0
    #total = 0
    #output_folder = output_folder + 'check_atlas_target_13_rest_4_to_12_CL_' + str(index)
    #output_folder = output_folder + 'check_atlas_target_'+str(index)+'_rest_LOOCLSIG'
    output_folder = output_folder + 'check_atlas_target_'+str(index)+'_rest_affine'
    ...
    for epoch in range(para.solver.epochs):
        ...
        optimizer.zero_grad()
        optimizer2.zero_grad()
        contrastive_loss_value = para.model.contrastive_weight*contrastive_loss_pos_neg_sigmoid(latent, latent_rest)
...
def main():
    ...
    for i in range(4,10):
        ...
        para.model.contrastive_weight = contrastive_weight_index[3]
        ...
        dataset = SData('./data_json/data_target_'+str(i)+'_rest_LOO_maleAffine.json', "train")
        dataset_rest = SData('./data_json/data_target_'+str(i)+'_rest_LOO_maleAffine.json', "train_rest")
        ave_data = SData('./data_json/data_target_'+str(i)+'_rest_LOO_maleAffine.json', 'linear')
        ...
        print (xDim, yDim, zDim)
        train_network(i, output_folder, trainloader, aveloader, train_restloader, net, net_rest, para, criterion, optimizer, optimizer2, para.solver.atlas_lr, NCC, 'l2', para.model.dist, para.model.reg, xDim, yDim, zDim, im_info, dev)
