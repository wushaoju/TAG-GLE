...
def train_network(output_folder, trainloader, aveloader, train_restloader, net, net_rest, para, criterion, optimizer, at_lr, DistType, RegularityType, weight_dist, weight_reg, xDim, yDim, zDim, im_info, dev):
    running_loss = 0
    total = 0
    output_folder = output_folder + 'check_atlas_target_13_rest_4_to_12'
    ...
    for epoch in range(para.solver.epochs):
        ...
        for j, tar_bch in enumerate(trainloader):
           ...
           # print (latent.shape)
            ...
            Dist = criterion(Idef, tar_bch)
            loss_total = (1/(para.solver.Sigma*para.solver.Sigma))*Dist + reg_term
        ...
        #     Dist_rest = criterion(Idef, tar_bch)
        #     loss_total_rest = (1/(para.solver.Sigma*para.solver.Sigma))*Dist_rest + reg_term
...
def main():
    ...
    print (xDim, yDim, zDim)
    train_network(output_folder, trainloader, aveloader, train_restloader, net, net_rest, para, criterion, optimizer, para.solver.atlas_lr, NCC, 'l2', para.model.dist, para.model.reg, xDim, yDim, zDim, im_info, dev)
